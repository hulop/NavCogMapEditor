	...
	function showAttributes(loc) {
		...
		for(var key in attrs) {
			...
			if (attrs[key].type == "Number" || attrs[key].type == "Text") {
				...
				var $download = $("<button>").text($i18n.t("download"));
				if (loc && loc[key]) {
				...
				$target.append($size);
				$target.append("<br>");
				$target.append($("<label>").text($i18n.t("Download")+": "));
				$target.append($download);
				$download.click((function(data, name) {
					return function(e) {
						downloadFileType(data, name+".txt", ["text/plain", "text"]);
					}
				})(loc[key], loc.name));
			}
